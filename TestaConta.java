public class TestaConta {
    public static void main(String[] args) {
        ...
        ContaSalario cs1 = new ContaSalario(33, 3, "Banco Inter", 100.00, 50, 20, 10);
        System.out.println(cs1);

        System.out.println("O saldo da conta salário é de R$ " + cs1.getSaldo());

        if (cs1.getSaldo() >= cs1.getSacar()) {
            System.out.println("Saldo da conta atual depois do saque R$ " + (cs1.getSaldo() - cs1.getSacar()));
        } else {
            System.out.println("Valor de saque indisponivel por falta de saldo na conta. Valor de saque disponivel R$ " + cs1.getSaldo());
        }

        if (cs1.getDepositar() <= 0) {
            System.out.println("Valor para ser depositado indisponivel ");
        } else {
            System.out.println("Deposito efetuado com sucesso. Seu saldo após deposito R$ " + (cs1.getSaldoAtualizado()));
        }


        if (cs1.getSaldoAtualizado() >= cs1.getSaque2()) {
            System.out.println("Saldo da conta atual depois do segundo saque R$ " + (cs1.getSaldoAtualizado() - cs1.getSaque2()));
        } else {
            System.out.println("Valor de saque indisponivel por falta de saldo na conta. Valor de saque disponivel R$ " + cs1.getSaldoAtualizado());
        }

        System.out.println("Saldo atualizado é de R$ " + cs1.getSaldoAtualizado2());

        System.out.println("Limite diario de saque atingido, tente novamente amanhã");
